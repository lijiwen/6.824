...
func doMap(
	...
	fmt.Printf("map is %d,reduce is %d", mapTaskNumber, nReduce)
	...
	cacheFile := make(map[int][]KeyValue)
	for _, value := range result {
		i := ihash(value.Key) % nReduce
		if _, ok := cacheFile[i]; !ok {
			k := make([]KeyValue, 1)
			k = append(k, value)
			cacheFile[i] = k
		} else {
